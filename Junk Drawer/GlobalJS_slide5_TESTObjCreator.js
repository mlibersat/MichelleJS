...
function ggbOnInit(name, ggbObject) {
  ...
  */
    ...
    let allPolys = [];
    let allPolySegs = [];
    let separatePolySegs = [];
    let allWhitePolySegs = [];
    let separateWhitePolySegs = [];
    let tempMdpt = "";
    ...
    let polyDef = "";
    let allPolyDefsArray = [];
    let whitePolyDef = "";
    let allWhitePolyDefsArray = [];
    ...
    function clickListenerFunction(a) {
      ...
      if (createdPolys.includes(clickedObject)) {
        if (editVertsToolActive) {
          ...
          console.warn("call getSegsArray - from click Listener");
    ...
    function makePoints() {
      ...
      if (addVertsToolActive) {
        ...
        if (spaceFlag) {
          ...
          console.log("selectedObject - should be a segment", selectedObject);
          makeAddedPointObj2(newPointName, selectedObject);
        } else {
          ...
          makeAddedPointObj2(newPointName, hoverSeg);
        }
        ggbObject.setColor(newPointName, 0, 0, 0);
        // makeAddedPointObj2(newPointName);
      }
    ...
    function makePoly() {
      ...
      console.log("tempPointsArray", tempPointsArray);
      console.warn("in Make Polys polyDef");
      // create the poly with non-intersecting sides using the new order of points in tempPointsArray
      polyDef = ggbObject.evalCommandGetLabels(newPolyName.concat("=Polygon(", tempPointsArray, ")"));
      console.log("polyDef", polyDef);
      allPolyDefsArray.push(polyDef);
      console.log("allPolyDefsArray", allPolyDefsArray);
      ...
      whitePolyDef = ggbObject.evalCommandGetLabels(newWhitePolyName.concat("=Polygon(", tempPointsArray, ")"));
      allWhitePolyDefsArray.push(whitePolyDef);
      console.log("allWhitePolyDefsArray", allWhitePolyDefsArray);
      ...
      allPolySegs.forEach(function (element) {
        ...
        // ggbObject.setLayer(whitePoly, whitePolyLayer);
        // ggbObject.setColor(whitePoly, 255, 255, 255);
        // ggbObject.setFixed(whitePoly, false, false);
        // ggbObject.setLineThickness(whitePoly, whiteLineThickness);
      });
    ...
    //   // console.warn("updateAllPolySegs");

    //   const segmentsToIgnore = ["buttonBarBorder", "fakeXaxis", "fakeYaxis"];
    //   const allGGBSegs = ggbObject.getAllObjectNames("segment");

    //   allPolys = detectVisiblePolys();
    //   let sortedSegs = allGGBSegs.filter(function (el) {
    //     // console.log(el);
    //     // return !segmentsToIgnore.includes(el);
    //     return el.includes("newpoint") || el.includes("newmdpt");
    //   });

    //   // let sortedWhiteSegs = allGGBSegs.filter(function (el) {
    //   //   console.log(el);
    //   //   return !segmentsToIgnore.includes(el) && !el.includes("newpoint") && !el.includes("newmdpt");
    //   // });

    //   // console.log("sortedSegs", sortedSegs);
    //   // console.log("sortedWhiteSegs", sortedWhiteSegs);
    //   let sortedByPolyDef = [];
    //   // let sortedWhiteByPolyDef = [];

    //   allPolySegs = [];
    //   separatePolySegs = [];
    //   // allWhitePolySegs = [];
    //   // separateWhitePolySegs = [];

    //   allPolys.forEach(function (element) {
    //     // const whitePoly = element.concat("White");

    //     sortedByPolyDef = getSegsArrayFromPolyName(element);
    //     // sortedByPolyDef = [...getSegsArrayFromPolyName(element)];
    //     separatePolySegs.push(sortedByPolyDef);
    //     allPolySegs.push(...sortedByPolyDef);

    //     // sortedWhiteByPolyDef = [...getSegsArrayFromPolyName(whitePoly)];
    //     // separateWhitePolySegs.push(sortedWhiteByPolyDef);
    //     // allWhitePolySegs.push(...sortedWhiteByPolyDef);
    //   });

    //   // console.log("separatePolySegs", separatePolySegs);
    //   // console.log("allPolySegs", allPolySegs);
    //   // console.log("separateWhitePolySegs", separateWhitePolySegs);
    //   // console.log("allWhitePolySegs", allWhitePolySegs);
    ...
    function detectAndStyleActivePolyPoints(obj) {
      console.warn("in detectAndStyleActivePolyPOINTS");
    ...
    function detectAndStyleActivePolySegs(obj) {
      ...
      if (polySelected || segSelected) {
        ...
        console.warn("call getSegsArray - from detectAndStyle");
    ...
    function getEndpointArrayFromSegName(seg) {
      ...
      console.log("end getEndpointArrayFromSegName, return:", segEndpointString.split(", "));
    ...
    function getPointsArrayFromPolyName(poly) {
      ...
      console.log("end getPointsArrayFromPolyName, return:", polyPointsString.split(", "));
    ...
    function getSegsArrayFromPolyName(poly) {
      // const polyDefString = ggbObject.getDefinitionString(poly);
      // console.warn("In getSegsArray, poly", poly);
      // const polySegsString = polyDefString.substring(8, polyDefString.length);
      // // console.log("b. in getSegsArray, polySegsString:", polySegsString);
      // // const polySegsString = polyDefString.substring(9, polyDefString.length - 1);
      // const segs = polySegsString.toLowerCase();
      // // console.log("c. in getSegsArray, segs:", segs);
      // // console.log("d. in getSegsArray, segs.split:", segs.split(", "));
      // console.log("end getSegsArrayFromPolyName, return:", segs.split(", "));
      // return segs.split(", ");

      // console.log("allPolyDefs,", allPolyDefsArray);
      let currentPolyDefStr = "";
      allPolyDefsArray.forEach(function (element) {
        if (element.includes(poly)) {
          currentPolyDefStr = element;
        }
      });

      const array = currentPolyDefStr.split(",");
    ...
    function manageToolChange() {
      ...
      createdPolys.forEach(function (element) {
        console.log("in manageToolChange - setFixed,", element, selectableOrNot);
    ...
    function bringSegsUpDown() {
      let selectableBool = addVertsToolActive ? true : false;
      ...
      // console.warn("allWhiteSegs", allWhiteSegs);
    ...
    function manageTabOrder(obj) {
      ...
      if (editVertsToolActive) {
        console.warn("in manageToolChange - editVertsToolActive");
        // const allPoints = ggbObject.getAllObjectNames("point");
        // // filter out anything that doesn't contain "NewPoint" and anything that does contain "White"
 ...
 */
    ...
    function AddedPoint(name, seg) {
      // this.name = "AddedPoint".concat(addedCoutner);
      // this.name = name.concat(addedCounter);
      this.name = name;
    ...
    function makeAddedPointObj2(pt, seg) {
      addedCoutner++;
    ...
    function addVerticesToPoly() {
      ...
      console.warn("in addVerticesToPoly function");
      // console.log("addedList", addedList);
      // console.log("addedPtObjArray", addedPtObjArray);
      // console.log("allPolyPoints", allPolyPoints);
      ...
      console.warn("call getSegsArray - from addVertices function");

      allPolys = detectVisiblePolys();
      allPolys.forEach(function (poly) {
        const points = [...getPointsArrayFromPolyName(poly)];
        const segs = [...getSegsArrayFromPolyName(poly)];
        //    a. addedPtObjArray locates the points each AddedPoint is between between
        //       (Opt 2, if object doesnt work: detect iscollinear, and x and y coords are between the endpoints of the segment)
        ...
        ggbObject.evalCommand(poly + "= Polygon(" + pointString + ")");
        console.log("polyDef: ", ggbObject.getDefinitionString(poly));
      });
