...
//////////////////////From IM1 M2 L1 - adding rows as they are verified (with Dane) //////////////////////////////
const {
  ggb1,
  text1,
  text2,
...
if (num == id1PrevTable1.data.goBackStringKatex || num2 == id1PrevTable1.data.goBackStringKatex) {
...
if (num3 == id1PrevTable2.data.goBackStringKatex || num4 == id1PrevTable2.data.goBackStringKatex) {
...
if (num7 == id2PrevTable1.data.goBackStringKatex || num8 == id2PrevTable1.data.goBackStringKatex) {
...
if (num9 == id2PrevTable1.data.goBackStringKatex || num10 == id2PrevTable1.data.goBackStringKatex) {
...
if (num11 == id2PrevTable1.data.goBackStringKatex || num12 == id2PrevTable1.data.goBackStringKatex) {
...
// table1.addRow(1, { math: true, editable: true });
console.log(table1);
console.log("table length", table1.data.rows.length);

let tableLength = table1.data.rows.length;
for (let i = 1; i < tableLength; i++) {
  table1.deleteRow(i);
}

let xLength = xVals.length + 1;
for (let i = 1; i < xLength; i++) {
  if (xVals[i - 1] == id1PrevTable1.data.goBackStringKatex || xVals[i - 1] == id2PrevTable1.data.goBackStringKatex) {
    table1.addRow(i, { math: false, editable: false });
    table1.updateCell(i, 0, { value: `${xVals[i - 1]}` });
    table1.updateCell(i, 1, { value: `${yVals[i - 1]}` });
    table1.updateCell(i, 2, { value: `${xVals[i - 1]}` });
  } else {
    table1.addRow(i, { math: true, editable: false });
    table1.updateCell(i, 0, { value: `${xVals[i - 1]}` });
    table1.updateCell(i, 1, { value: `${yVals[i - 1]}` });
    table1.updateCell(i, 2, { value: `(${xVals[i - 1]},${yVals[i - 1]})` });
  }
}
