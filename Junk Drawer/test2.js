function ggbOnInit(name, ggbObject) {
  loadUtils().then(function (setupGGB) {
    ...
    function defineKeyboardInstructions(obj) {
      ...
      const keyboardInstructions = {
        ...
        ggbButton1: ggbObject.getValue("ggbButton1Enabled")
          ? "Press space to draw dots in the place value chart."
          : unavailableButtonText,
        ...
        ggbButton3: ggbObject.getValue("ggbButton3Enabled")
          ? "Press space to reset the interactive."
          : unavailableButtonText,
    ...
    const altText = {
      appletStatusSentences: {
        ...
        addSentence: function (string) {
          if (string.length > 0) this.statusArray.push(string);
        },
        readNext: function () {
      ...
      buildAppletStatus: function () {
        ...
        ggbObject.setTextValue(
          "AAppletStatus",
          "A place value chart shows One, Ten, Hundred, Thousand, TenThousand, and HundredThousand. Press tab to select next object. Press space to continue reading describtion. Press the escape key to exit the interactive and return to the page."
        );
        ...
        var ss = "";
        ss +=
          " To the right of the place value chart " +
          this.numberToString(firstNumber) +
          " minus " +
          secondNumber +
          " in vertical form.";
      ...
      getLeftPartSentence: function () {
        try {
          ...
          let mil =
            deletedPoints.millions > 0
              ? ggbObject.getValue(variables.milCount3)
              : ggbObject.getValue(variables.milCount);
          let hundredthousand =
            deletedPoints.hundredThousands > 0
              ? ggbObject.getValue(variables.hunthouCount3)
              : ggbObject.getValue(variables.hunthouCount);
          let tenthousand =
            deletedPoints.TenThousands > 0
              ? ggbObject.getValue(variables.tenthouCount3)
              : ggbObject.getValue(variables.tenthouCount);
          let thousand =
            deletedPoints.Thousands > 0
              ? ggbObject.getValue(variables.thouCount3)
              : ggbObject.getValue(variables.thouCount);
          let hundreds =
            deletedPoints.hundereds > 0
              ? ggbObject.getValue(variables.hunsCount3)
              : ggbObject.getValue(variables.hunsCount);
          let ten =
            deletedPoints.tens > 0 ? ggbObject.getValue(variables.tensCount3) : ggbObject.getValue(variables.tensCount);
          let one =
            deletedPoints.tens > 0 ? ggbObject.getValue(variables.onesCount3) : ggbObject.getValue(variables.onesCount);
          ...
          s +=
            " To the right of the place value chart " + firstNumber + " minus " + secondNumber + " in vertical form.";
          return s;
        } catch (error) {
          console.log(error);
        }
        ...
        try {
          ...
          if (hundredthousand > 0) {
            s +=
              ...
              hundredthousand +
              " dot" +
              this.s(hundredthousand) +
              " in the hundred thousand column " +
              this.isAre(hundredthousand) +
              " crossed off.";
          }
          if (tenthousand > 0) {
            s +=
      ...
      getPointSenteceForCaption: function (pointName) {
        ...
        if (pointName.includes("HT")) {
          let string =
            "Dot in the Hundred Thousands column is selected. Press right arrow to regroup 1 hundred thousand. ";
          return string;
        }
        if (pointName.includes("TT")) {
      ...
      if (grabbed.type == "dragEnd") {
          ...
          if (
            ...
            for (var column = 0; column < 2; column++) {
              for (var row = 0; row < 5; row++) {
                ggbObject.evalCommand("HT" + labelHunThou + "=PointIn(region6)");
                altText.setPointCaption("HT" + labelHunThou);
                ggbObject.setCoords(
                  "HT" + labelHunThou,
                  ggbObject.getXcoord("Ig23") + row,
                  ggbObject.getYcoord("Ig23") - 3 - column - rowAdderHunThou
                );
                ggbObject.setColor("HT" + labelHunThou, 255, 218, 0);
                ...
                ggbObject.setValue("hunthouCount3", pointsHT.length);
        ...
        if (type == "point") {
          if (
            ...
            for (var column = 0; column < 2; column++) {
              for (var row = 0; row < 5; row++) {
                ggbObject.evalCommand("TT" + labelTenThou + "=PointIn(region5)");
                altText.setPointCaption("TT" + labelTenThou);

                ggbObject.setCoords(
                  "TT" + labelTenThou,
                  ggbObject.getXcoord("Ig22") + row,
                  ggbObject.getYcoord("Ig22") - 3 - column - rowAdderTenThou
                );
                ggbObject.setColor("TT" + labelTenThou, 241, 127, 9);
                ...
                ggbObject.setValue("hunthouCount3", pointsHT.length);
                ggbObject.setValue("tenthouCount3", pointsTT.length);
                ggbObject.setVisible("k", true);
        ...
        if (type == "point") {
          if (
            ...
            for (var column = 0; column < 2; column++) {
              for (var row = 0; row < 5; row++) {
                ggbObject.evalCommand("TH" + labelThous + "=PointIn(region4)");
                altText.setPointCaption("TH" + labelThous);

                ggbObject.setCoords(
                  "TH" + labelThous,
                  ggbObject.getXcoord("Ig21") + row,
                  ggbObject.getYcoord("Ig21") - 3 - column - rowAdderThous
                );
                ggbObject.setColor("TH" + labelThous, 120, 120, 120);
        ...
        if (type == "point") {
          if (
            ...
            for (var column = 0; column < 2; column++) {
              for (var row = 0; row < 5; row++) {
                ggbObject.evalCommand("HU" + labelHuns + "=PointIn(region3)");
                altText.setPointCaption("HU" + labelHuns);

                ggbObject.setCoords(
                  "HU" + labelHuns,
                  ggbObject.getXcoord("Ig5") + row,
                  ggbObject.getYcoord("Ig5") - 3 - column - rowAdderHuns
                );
        ...
        if (type == "point") {
          if (
            ...
            for (var column = 0; column < 2; column++) {
              for (var row = 0; row < 5; row++) {
                ggbObject.evalCommand("TE" + labelTens + " =PointIn(region2)");
                altText.setPointCaption("TE" + labelTens);

                ggbObject.setCoords(
                  "TE" + labelTens,
                  ggbObject.getXcoord("Ig6") + row,
                  ggbObject.getYcoord("Ig6") - 3 - column - rowAdderTens
                );
        ...
        if (type == "point") {
          if (
            ...
            for (var column = 0; column < 2; column++) {
              for (var row = 0; row < 5; row++) {
                ggbObject.evalCommand("O" + labelOnes + "=PointIn(region1)");
                altText.setPointCaption("O" + labelOnes);

                ggbObject.setCoords(
                  "O" + labelOnes,
                  ggbObject.getXcoord("Ig7") + row,
                  ggbObject.getYcoord("Ig7") - 3 - column - rowAdderOnes
                );
      ...
      try {
          ...
          for (var popLoop = 0; popLoop < labelMil2; popLoop++) {
            ...
            if (labelMil2 == 1) {
              ggbObject.evalCommand(
                "MBCross" + popLoop + "=Segment(MB" + popLoop + "-(0.5,0.5),MB" + popLoop + "+(0.5,0.5))"
              );
              ...
              ggbObject.evalCommand(
                "MBCross" + popLoop + "=Segment(MB" + popLoop + "+(0.5,0),MB" + popLoop + "-(0.5,0))"
              );
              ggbObject.setFixed("MBCross" + popLoop, true, false);
              ggbObject.setLayer("MBCross" + popLoop, 2);
              ggbObject.setColor("MBCross" + popLoop, 0, 0, 0);
        ...
        if (pointsHT.length >= labelHunThou2) {
          ...
          for (var popLoop = 0; popLoop < labelHunThou2; popLoop++) {
            ...
            if (labelHunThou2 == 1) {
              ggbObject.evalCommand(
                "HTBCross" + popLoop + "=Segment(HTB" + popLoop + "-(0.5,0.5),HTB" + popLoop + "+(0.5,0.5))"
              );
              ...
              ggbObject.evalCommand(
                "HTBCross" + popLoop + "=Segment(HTB" + popLoop + "+(0.5,0),HTB" + popLoop + "-(0.5,0))"
              );
              ggbObject.setFixed("HTBCross" + popLoop, true, false);
              ggbObject.setLayer("HTBCross" + popLoop, 2);
              ggbObject.setColor("HTBCross" + popLoop, 0, 0, 0);
        ...
        if (pointsTT.length >= labelTenThou2) {
          ...
          for (var popLoop = 0; popLoop < labelTenThou2; popLoop++) {
            ...
            if (labelTenThou2 == 1) {
              ggbObject.evalCommand(
                "TTBCross" + popLoop + "=Segment(TTB" + popLoop + "-(0.5,0.5),TTB" + popLoop + "+(0.5,0.5))"
              );
              ...
              ggbObject.evalCommand(
                "TTBCross" + popLoop + "=Segment(TTB" + popLoop + "+(0.5,0),TTB" + popLoop + "-(0.5,0))"
              );
              ggbObject.setFixed("TTBCross" + popLoop, true, false);
              ggbObject.setLayer("TTBCross" + popLoop, 2);
              ggbObject.setColor("TTBCross" + popLoop, 0, 0, 0);
        ...
        if (pointsTH.length >= labelThous2) {
          ...
          for (var popLoop = 0; popLoop < labelThous2; popLoop++) {
            ...
            if (labelThous2 == 1) {
              ggbObject.evalCommand(
                "THBCross" + popLoop + "=Segment(THB" + popLoop + "-(0.5,0.5),THB" + popLoop + "+(0.5,0.5))"
              );
              ...
              ggbObject.evalCommand(
                "THBCross" + popLoop + "=Segment(THB" + popLoop + "+(0.5,0),THB" + popLoop + "-(0.5,0))"
              );
              ggbObject.setFixed("THBCross" + popLoop, true, false);
              ggbObject.setLayer("THBCross" + popLoop, 2);
              ggbObject.setColor("THBCross" + popLoop, 0, 0, 0);
    ...
    function setAnswerText() {
      ...
      if (ggbObject.getValue("milCount4") == 0 && ggbObject.getValue("hunthouCount4") == 0) {
        ggbObject.setVisible("textMil4", false);
        ggbObject.setVisible("textHunThou4", false);
        ggbObject.setVisible("textCommas4", false);
      }
    ...
    function reset() {
      ...
      ggbObject.setTextValue(
        "AAppletStatus",
        "A place value chart shows One, Ten, Hundred, Thousand, TenThousand, and HundredThousand. Press tab to select next object."
      );
      deletedPoints.reset();
      altText.reset();
      ...
      for (iter9 = 0; iter9 < lengthPointNames; iter9++) {
        if (
          !(
            pointNames[iter9].includes("Ig") ||
            pointNames[iter9] === "corner1" ||
            pointNames[iter9] === "corner2" ||
            pointNames[iter9] === "corner3" ||
            pointNames[iter9] === "corner4" ||
            pointNames[iter9] === "corner" ||
            pointNames[iter9] === "BottomLeftButtonBar" ||
            pointNames[iter9] === "BottomRightButtonBar" ||
            pointNames[iter9] === "InstructionsIconPoint" ||
            pointNames[iter9] === "XIconPoint" ||
            pointNames[iter9] === "KeyInstructionsBR" ||
            pointNames[iter9] === "KeyInstructionsTL" ||
            pointNames[iter9] === "Export_1" ||
            pointNames[iter9] === "Export_2" ||
            pointNames[iter9] === "HHH"
          )
        ) {
    ...
    function reset2() {
      reset();
      ggbObject.evalCommand("SetValue( subt1, ? )");
      ggbObject.evalCommand("SetValue( subt2, ? )");
    }
