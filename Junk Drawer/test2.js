...
function ggbOnInit(name, ggbObject) {
  loadUtils().then(function (setupGGB) {
    ...
    // setInitialTabOrder
    const initList = "FirstNumber, SecondNumber, ggbButton1, ggbButton2, ggbButton3";
    let added = "";
    ...
    function quickChange() {
      ...
      if (newNum !== oldNum || newNum2 !== oldNum2) {
        ...
        oldNum2 = newNum2;
        unbundling = false;
        console.log("resetting bundledRegions in quickChange");
    ...
    function organize(snapBackBundle = false) {
      ...
      // console.log(
      //   "In organize - selecting current object",
      //   currentObject,
      //   "caption",
      //   ggbObject.getCaption(currentObject)
      // );
      // ggbObject.evalCommand("SelectObjects(" + currentObject + ")");
      if (ggbObject.getCaption(currentObject) !== "") {
        ggbObject.evalCommand("SelectObjects(" + currentObject + ")");
      }
        ...
        if (element >= 10 && i <= numRegions - 1 && !nextRegionIsFull) {
          ...
          for (let i = 0, L = objectNames.length; i < L; i++) {
            if (
              ...
              console.log("visible polys", objectNames[i], ggbApplet.getVisible(objectNames[i]));
...
*/
      ...
      switch (true) {
        ...
        case bundling:
          {
            ...
            // console.log(
            //   "In bundling - selecting ",
            //   regions[regionNum + 1] + "Point" + (totalNumArray[regionNum + 1] + 1),
            //   "caption",
            //   ggbObject.getCaption(regions[regionNum + 1] + "Point" + (totalNumArray[regionNum + 1] + 1))
            // );
            ...
            if (ggbObject.getCaption(regions[regionNum + 1] + "Point" + (totalNumArray[regionNum + 1] + 1)) === "") {
              console.log("caption empty --", regions[regionNum + 1] + "Point" + (totalNumArray[regionNum + 1] + 1));
            }
            ggbObject.evalCommand(
              "SelectObjects(" + regions[regionNum + 1] + "Point" + (totalNumArray[regionNum + 1] + 1) + ")"
            );
        ...
        */
            ...
            regions.forEach(function (regionName, regionIndex) {
              ...
              console.warn("selectedObjectRegion", selectedObjectRegion);
    ...
    function returnMinMaxString() {
      ...
      console.log(
        selectedObject,
        selectedObjectXCoord,
        selectedObjectYCoord,
      ...
      if (startingPositionBundle) {
        console.warn("starting position bundle!!!");
      }
    ...
    function clientFunction(clientEvent) {
      ...
      switch (clientEvent.type) {
        case "select": {
          ...
          if (selectedObject.includes("Point")) {
            console.log(
              "In client event select point - ",
              selectedObject,
              "caption",
              ggbObject.getCaption(selectedObject)
            );
          }
          ...
          if (selectedObject.includes("Frame")) {
            ...
            // gets the index of the region from the const array "regions"
            // const regionNum = regions.indexOf(selectedObjectRegion);
            // const bundleIsImmediatelyToTheRightOfBundlingRegion =
            //   ggbObject.getXcoord(regions[regionNum] + "TenFrameCorner") -
            //     (numRegions - regionNum - 1) *
            //       ggbObject.getValue("sectionWidth") <=
            //   1;

            // spanish translations: selectedObjectRegion is one of the following: "One","Ten","Hundred","Thousand","TenThousand","HundredThousand","Million". And regions[regionNum + 1] is the next highest region in the list.
            // if (bundleIsImmediatelyToTheRightOfBundlingRegion) {
            //   ggbReadText(
            //     "Press the left arrow to move the bundle of 10 " +
            //       selectedObjectRegion +
            //       "s to the " +
            ...
            //       "s column."
            //   );
            //   console.log("select read text")
            // }
            // if (bundleIsImmediatelyToTheRightOfBundlingRegion) {
            ggbReadText(returnMinMaxString());
            // }
          } else if (pointsAllowedToUnbundle.includes(selectedObject)) {
            ggbReadText("Press u to unbundle this point." + returnMinMaxString());
          }
          // } catch (error) {
          //   console.error(error);
          // }
        ...
        case "dragEnd": {
          ...
          console.warn("totalNumArray in dragEnd", totalNumArray);
          performSwap(totalNumArray);
          preXCoord = 0;
          preYCoord = 0;
          if (selectedObject.includes("Point") || selectedObject.includes("Frame")) {
            setTimeout(function () {
              ggbReadText(returnMinMaxString());
            }, 500);
          }
