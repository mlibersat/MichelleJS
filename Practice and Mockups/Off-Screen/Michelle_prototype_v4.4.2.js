function ggbOnInit(name, ggbObject) {
  loadUtils().then(function (setupGGB) {
    ...
    function updateFirstLastIndex() {
      bIsLast = ggbObject.getValue("bIsLastIndex"); // COME BACK TO THIS <- BIsLastIndex doesn't show true because "index" command never gets to 144
      ...
      switch (a.type) {
        case "select":
          ...
          switch (selectedObject) {
            case "A":
              ...
              ggbObject.setValue("showFakeAB", true);
              break;
            case "B":
        ...
        case "deselect":
          ...
          if (selectedObject === "A") {
            ...
            ggbObject.setValue("showFakeAB", false);
          }
          selectedObject = "";
          ggbObject.setValue("showFakeAB", false); // COME BACK TO THIS
    ...
    function repositionB() {
      // const coordPair = "(" + ggbObject.getXcoord("BFake") + "," + ggbObject.getYcoord("BFake") + ")";
      // const indexBFake = ggbObject.evalCommand("IndexOf(" + coordPair + ", circlePathBRestrictedNew)");
      // ggbObject.evalCommand("B=(7,0)");
      // ggbObject.evalCommand("B=Point(circlePathBRestrictedNew)");
    ...
    function keyit(event) {
      ...
      if (selectedObject === "B" && (key === "+" || key === "-" || key.includes("Arrow") || key === "Tab")) {
        console.log("in keyit. B selected. key", key);
        repositionStuff();

        // // updateFirstLastIndex();
        // const firstPositionX = ggbObject.getXcoord("BRestrictedFirst");
        // const firstPositionY = ggbObject.getYcoord("BRestrictedFirst");
        // const lastPositionX = ggbObject.getXcoord("BRestrictedLast");
        // const lastPositionY = ggbObject.getYcoord("BRestrictedLast");

        // If B is the first or last element in the path list, allow B to loop around the list on press + or -
        //////////////////COME BACK TO THIS!!!!!/////////////// - SKIPS OVER
        // updateFirstLastIndex();
        ...
        console.log("in keyit. A selected. key", key);
        repositionB();
      }
      // updateFirstLastIndex();
    }
